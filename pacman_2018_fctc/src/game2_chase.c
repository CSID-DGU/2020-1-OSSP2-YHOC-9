...
void game_tick2(PacmanGame2 *game)
{
	...
	case GamePlayState2:
		...
		if (game->pacman.livesLeft != -1)
			process_player2(&game->pacman, &game->board[game->stageLevel], One1);
		if (game->mode != SoloState && game->pacman_enemy.livesLeft != -1)
			process_player2(&game->pacman_enemy, &game->board[game->stageLevel], Two2);

		//process_player2(&game->pacman, &game->board, One1);
		//if(game->mode != SoloState) process_player2(&game->pacman_enemy, &game->board, Two2);

		if (game->pacman.missile == 1)
			process_missiles2(game);
		if (game->mode != SoloState && game->pacman_enemy.missile == 1)
			process_missiles2(game);
		...
		if (game->mode != SoloState && (game->pacman_enemy.score > game->highscore))
			game->highscore = game->pacman_enemy.score;
	...
	if (game->mode != SoloState)
		player2_lives = game->pacman_enemy.livesLeft;
	...
	case GamePlayState2:
		...
		if (key_held(SDLK_k))
			enter_state2(game, DeathState2);
		else if (allPelletsEaten)
			enter_state2(game, WinState2);

		if (collidedWithchaser == 1)
			enter_state2(game, DeathState2);
		else if (collidedWithchaser == 2)
			enter_state2(game, DeathState2);
		break;
	case WinState2:
	...
	case DeathState2:
		if (dt > 4000)
		{
			if (game->mode == SoloState && lives == 0)
...
void game_render2(PacmanGame2 *game, int tick)
{
	...
	if (game->mode != SoloState)
		draw_common_twoup(true, game->pacman_enemy.score);

	draw_common_highscore(game->highscore);

	draw_pacman_lives(game->pacman.livesLeft);
	if (game->mode != SoloState)
		draw_pacman2_lives(game->pacman_enemy.livesLeft);
	...
	case LevelBeginState2:
		...
		if (game->mode != SoloState && game->pacman_enemy.livesLeft != -1)
			draw_pacman2_static(&game->pacman_enemy);
	...
	case GamePlayState2:
		...
		if (game->mode != SoloState)
		{
			...
			if (game->pacman.godMode)
			{
				//printf("scared\n");
				if(!draw_scared_chaser(&game->pacman_enemy, godDt)){
					game->pacman.godMode = false;
					godChange = false;
				}
			}
			else if (game->mode != SoloState && game->pacman_enemy.livesLeft != -1)
			{
				//printf("ok\n");
	...
	case DeathState2:
		...
		else
		{
			...
			if (game->mode != SoloState && game->pacman_enemy.livesLeft != -1)
			{
				if (game->mode != SoloState && game->death_player == Two2)
					draw_pacman2_death(&game->pacman_enemy, dt - 1000);
				else
					draw_pacman2_static(&game->pacman_enemy);
			}
...
static void enter_state2(PacmanGame2 *game, GameState2 state)
{
	...
	if (game->pacman.score > origin_highscore)
		writeScoreToFile(game->pacman.score);
...
void gamestart_init2(PacmanGame2 *game, int mode)
{
	...
	if (mode == SoloState)
		game->mode = SoloState;
	else if (mode == MultiState)
		game->mode = MultiState;
	else
		game->mode = RemoteState;
	...
	if (game->mode != SoloState)
		pacman_chaser_init(&game->pacman_enemy); ////////////////////////////////////////here
...
void level_init2(PacmanGame2 *game)
{
	...
	if (game->mode != SoloState)
		pacman_level_init(&game->pacman_enemy, 2);
